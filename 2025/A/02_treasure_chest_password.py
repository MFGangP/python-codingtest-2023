...
for testcase in range(1, T+1):
    # N은 숫자의 개수, K는 크기 순서(K번째로 큰 수 찾기)
    N, K = map(int, input().split())
    # 입력받은 16진수 
    words = list(input())
    # 반복할 횟수
    # N//4한 수에 -1번 하면 총 회전 수
    repeat_time = (N//4)
    passwords = []

    # 한바퀴 돌면 배치가 똑같아진다.
    for start in range(repeat_time):
        # words 길이 만큼 돌면서 값을 한 칸씩 당기기
        # repeat_time 만큼 반복하면 처음 상태로 돌아온다.
        for i in range(len(words)-1):    
            temp = words[i%N]        
            words[i%N] = words[(i+1)%N]
            words[(i+1)%N] = temp
        # 4칸씩 끊어서 password 리스트에 저장